...
// integration tests). Cargo's `cargo:rustc-link-arg` from dm_noesis_runtime's build.rs
...
//
// dm_noesis_runtime publishes the resolved Bin/<platform> path as DEP_NOESIS_LIB_DIR
...
fn main() {
    ...
    let lib_dir = std::env::var("DEP_NOESIS_LIB_DIR").expect(
        "DEP_NOESIS_LIB_DIR not set — dm_noesis_runtime's build.rs should emit it via \
         `cargo:lib_dir=...`. Did the dm_noesis_runtime dependency build?",
    );
